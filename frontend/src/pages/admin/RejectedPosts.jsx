...
import { Check, XCircle, ShieldAlert, FileText, Eye } from 'lucide-react';
...
const RejectedPosts = () => {
    ...
    useEffect(() => {
        const fetchRejected = async () => {
            try {
                const { data } = await API.get('/admin/rejected');
                setPosts(data.posts);
            } catch (err) {
    ...
    if (loading) {
        return <div className="loading-container"><div className="spinner"></div><div className="loading-text">Loading rejected posts...</div></div>;
    }
        ...
        <div>
            ...
            >
                <h1 style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <XCircle size={24} /> Rejected Posts
                </h1>
                <p>Posts auto-rejected or manually rejected by admin</p>
            </motion.div>
            ...
            {posts.length === 0 ? (
                <div className="empty-state">
                    ...
                    <p>All clear on the moderation front</p>
                </div>
            ) : (
                        ...
                        <tbody>
                            {posts.map(post => (
                                ...
                                >
                                    ...
                                    <td>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.8rem', color: 'var(--rose-400)' }}>
                                            <ShieldAlert size={13} />
                                            <span className="truncate" style={{ maxWidth: '180px' }}>{post.scam_reason || 'Manual rejection'}</span>
                                        </div>
                                    </td>
                                    <td>
                                        <span className="font-mono text-sm">{((post.scam_confidence || 0) * 100).toFixed(0)}%</span>
                                    </td>
                                    <td>
                    ...
                    >
                        ...
                        >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                                <h3 style={{ margin: 0, color: 'var(--text-primary)' }}>Rejected Post Details</h3>
                            ...
                            {/* Scam Info */}
                            <div style={{ background: 'var(--surface-input)', borderRadius: 'var(--r-lg)', padding: '12px', marginBottom: '16px', border: '1px solid rgba(244,63,94,0.2)' }}>
                                <div style={{ fontSize: '0.75rem', color: 'var(--rose-400)', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                ...
                                <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                                    <strong>Reason:</strong> {viewPost.scam_reason || 'Manual rejection'}
                                </div>
                            </div>

                            {/* Actions */}
                            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                                <button className="btn btn-success btn-sm" onClick={() => handleApprove(viewPost.id)}>
                                    <Check size={14} /> Restore Post
                                </button>
                            </div>
                        </motion.div>
                    </motion.div>
